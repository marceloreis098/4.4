...
app.post('/api/approvals/approve', async (req, res) => {
    ...
    try {
        await db.promise().query(`UPDATE ${tableName} SET approval_status = 'approved' WHERE id = ?`, [id]);
...
app.post('/api/approvals/reject', async (req, res) => {
    ...
    const tableName = type === 'equipment' ? 'equipment' : 'licenses';
    try {
        await db.promise().query(`UPDATE ${tableName} SET approval_status = 'rejected', rejection_reason = ? WHERE id = ?`, [reason, id]);
...
app.post('/api/enable-2fa', (req, res) => {
    ...
    db.query('SELECT twoFASecret FROM users WHERE id = ?', [userId], (err, results) => {
        ...
        const secret = results[0].twoFASecret;
        const isValid = authenticator.check(token, secret);

        if (isValid) {
            db.query('UPDATE users SET is2FAEnabled = TRUE WHERE id = ?', [userId], (updateErr) => {
                if (updateErr) return res.status(500).json({ message: 'Falha ao habilitar 2FA no banco de dados.' });
                res.status(204).send();
            });
        } else {
...
app.post('/api/disable-2fa', (req, res) => {
    const { userId } = req.body;
    // Here, you might want to add another verification step, like asking for the user's password.
    ...
    db.query('UPDATE users SET is2FAEnabled = FALSE, twoFASecret = NULL WHERE id = ?', [userId], (err) => {
        if (err) return res.status(500).json({ message: 'Falha ao desabilitar 2FA.' });
        res.status(204).send();
    });
});
...
app.post('/api/disable-user-2fa', (req, res) => {
    const { userId } = req.body;
    // This endpoint should be protected and only accessible by an admin. We'll rely on the `isAdmin` middleware.
    db.query('UPDATE users SET is2FAEnabled = FALSE, twoFASecret = NULL WHERE id = ?', [userId], (err) => {
        if (err) return res.status(500).json({ message: 'Falha ao desabilitar 2FA para o usuário.' });
        res.status(204).send();
    });
});
...
app.get('/api/settings', async (req, res) => {
    try {
        ...
        const settings = rows.reduce((acc, row) => {
            ...
            if (['isSsoEnabled', 'is2faEnabled', 'require2fa', 'smtpSecure', 'hasInitialConsolidationRun'].includes(row.config_key)) {
                value = value === 'true';
            }
            if (row.config_key === 'smtpPort' && value !== null) {
        ...
        res.status(500).json({ message: "Database error on getSettings", error: err });
    }
});
...
app.post('/api/settings', isAdmin, async (req, res) => {
    ...
    try {
        ...
        await connection.commit();
        logAction(username, 'SETTINGS_UPDATE', 'SETTINGS', 'ALL', `Updated application settings.`);
        res.json({ success: true, message: 'Configurações salvas.' }); // Send a JSON success response
    } catch (err) {
        await connection.rollback();
        console.error("Save settings error:", err);
        res.status(500).json({ success: false, message: "Database error on saveSettings", error: err }); // Send a JSON error response
    } finally {
        connection.release();
    }
});
...
app.get('/api/database/backup-status', isAdmin, (req, res) => {
    fs.readdir(BACKUP_DIR, (err, files) => {
        if (err) {
            return res.status(500).json({ hasBackup: false });
        }
        const backupFiles = files.filter(f => f.endsWith('.sql.gz')).sort().reverse();
        if (backupFiles.length > 0) {
            const latestBackup = backupFiles[0];
            const timestamp = latestBackup.replace('backup-', '').replace('.sql.gz', '');
            res.json({ hasBackup: true, backupTimestamp: new Date(timestamp).toISOString() });
        } else {
...
app.post('/api/database/backup', isAdmin, (req, res) => {
    ...
    const command = `mysqldump --user=${DB_USER} --password=${DB_PASSWORD} --host=${DB_HOST} ${DB_DATABASE}`;

    const dumpProcess = exec(command, (error, stdout, stderr) => {
        if (error) {
            console.error(`Backup exec error: ${error}`);
            return res.status(500).json({ success: false, message: 'Falha ao executar mysqldump.' });
        }
        if (stderr) {
            console.error(`Backup stderr: ${stderr}`);
            // Don't always fail on stderr, as it can contain warnings.
        }
        ...
        destination.on('finish', () => {
            fs.unlink(filepath, (unlinkErr) => { // Clean up the uncompressed file
                if (unlinkErr) console.error("Error deleting uncompressed backup file:", unlinkErr);
            });
            logAction(username, 'UPDATE', 'DATABASE', null, 'Database backup created.');
            res.json({ success: true, message: `Backup criado com sucesso: ${path.basename(compressedFilepath)}`, backupTimestamp: new Date().toISOString() });
        });

        destination.on('error', (compressErr) => {
            console.error("Compression error:", compressErr);
            fs.unlink(filepath, () => {}); // Attempt to clean up
            res.status(500).json({ success: false, message: 'Falha ao comprimir o backup.' });
        });
    });

    dumpProcess.stdout.pipe(fs.createWriteStream(filepath));
});
...
app.post('/api/database/restore', isAdmin, (req, res) => {
    const { username } = req.body;
    fs.readdir(BACKUP_DIR, (err, files) => {
        if (err) return res.status(500).json({ success: false, message: 'Não foi possível ler o diretório de backups.' });
        ...
        const latestBackup = backupFiles[0];
        const compressedFilepath = path.join(BACKUP_DIR, latestBackup);
        const uncompressedFilepath = compressedFilepath.replace('.gz', '');

        const gunzip = zlib.createGunzip();
        const source = fs.createReadStream(compressedFilepath);
        const destination = fs.createWriteStream(uncompressedFilepath);

        source.pipe(gunzip).pipe(destination);

        destination.on('finish', () => {
            const command = `mysql --user=${DB_USER} --password=${DB_PASSWORD} --host=${DB_HOST} ${DB_DATABASE} < ${uncompressedFilepath}`;
            exec(command, (error, stdout, stderr) => {
                fs.unlink(uncompressedFilepath, () => {}); // Clean up uncompressed file
                if (error) {
                    console.error(`Restore error: ${error}`);
                    return res.status(500).json({ success: false, message: `Falha na restauração: ${error.message}` });
                }
                logAction(username, 'UPDATE', 'DATABASE', null, `Database restored from ${latestBackup}.`);
                res.json({ success: true, message: 'Banco de dados restaurado com sucesso.' });
            });
        });
        destination.on('error', (uncompressErr) => {
            console.error("Uncompression error:", uncompressErr);
            res.status(500).json({ success: false, message: 'Falha ao descomprimir o backup.' });
        });
    });
});
...
app.post('/api/database/clear', isAdmin, async (req, res) => {
    ...
    try {
        ...
        // After clearing, re-run migrations to set up the clean schema
        await runMigrations();
        
        logAction(username, 'DELETE', 'DATABASE', 'ALL', 'Cleared and re-initialized the database.');
        res.json({ success: true, message: 'Banco de dados foi zerado e reinicializado.' });
